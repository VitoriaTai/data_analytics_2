...
select a.first_name, b.department, c.name from physician a
...
select patient.first_name, physician.first_name from patient
		join physician on patient.pcp = physician.employee_id; 
...
--find physician workload such as procedure time and doctors, nurse details
select a.first_name as doctor_name, b.first_name as nurse_name, c.date_time as proceduretime
		from physician a 
		join nurse b
		join undergoes c
...
select a.first_name, b.does, c.name, c.brand from patient a
		join prescribes b
		join medication c
		on c.code = b.medication
		on a.insurance_id = b.patient;
